...
if (!iSTUDENT) { redirect("../index.php"); }
...
// Przedmioty
$subjects = $userdata['student_subjects'];
...
opentable($locale['r13']." - ".$locale['r11']." ".$semestr);
	...
	// Zachowanie
	echo "<div align='center' style='margin:5px;'>".$locale['r32']." - ".os($userdata['os'.$semestr])."</div>";
	...
	while (isset($sub[$j]) && $sub[$j] != "") {
    	$data = dbarray(dbquery("SELECT ".$semestr."sem, ".$semestr."os FROM ".DB_PREFIX."sub".$sub[$j]." WHERE id_student='".$userdata['user_id']."'"));
 		$q1 = 0; $q2 = 0;
		$rat = explode("::", $data[$semestr.'sem']);
		
		// Opis oceny
		$res = dbquery("SELECT ".$semestr."sem FROM ".DB_PREFIX."sub".$sub[$j]."_descr WHERE d_class='".$userdata['student_class']."'");
		$dat = dbarray($res);
