...
if (iMEMBER) {
	...
	$orderby = ($_GET['sortby'] == "all" ? "" : " WHERE user_name LIKE '".stripinput($_GET['sortby'])."%' OR name LIKE '".stripinput($_GET['sortby'])."%' OR surname LIKE '".stripinput($_GET['sortby'])."%'");
	...
	if ($rows) {
		...
		echo "</tr>\n";
		$result = dbquery("SELECT * FROM ".DB_USERS.$orderby." ORDER BY user_level DESC, surname LIMIT ".$_GET['rowstart'].",20");
		while ($data = dbarray($result)) {
			$cell_color = ($i % 2 == 0 ? "tbl1" : "tbl2"); $i++;
			echo "<tr>\n<td class='$cell_color'>\n<a href='profile.php?lookup=".$data['user_id']."'>".$data['surname']." ".$data['name']." (".$data['user_name'].")</a></td>\n";
