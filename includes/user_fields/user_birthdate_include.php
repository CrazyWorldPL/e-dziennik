...
if ($profile_method == "input") {
	...
	$months = explode("|", $locale['uf_birthdate_months']);
	echo "<tr>\n";
	...
	for ($i = 1; $i <= 31; $i++) { echo "<option".($user_day == $i ? " selected='selected'" : "")." value='".$i."'>".$i."</option>\n"; }
	echo "</select>\n<select name='user_month' class='textbox'>\n<option value=''>&nbsp;</option>\n";
	for ($i = 1; $i <= 12; $i++) { echo "<option".($user_month == $i ? " selected='selected'" : "")." value='".$i."'>".$months[$i]."</option>\n"; }
	echo "</select>\n<select name='user_year' class='textbox'>\n<option value=''>&nbsp;</option>\n";
	for ($i = date("Y"); $i > 1930; $i--) { echo "<option".($user_year == $i ? " selected='selected'" : "")." value='".$i."'>".$i."</option>\n"; }
	echo "</select>\n</td>\n";
	echo "</tr>\n";
