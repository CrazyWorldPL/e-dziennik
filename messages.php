...
if (isset($_POST['send_message'])) {
	...
	if (iADMIN && isset($_POST['chk_sendtoall']) && isnum($_POST['msg_to_group'])) {
		...
		if (!flood_control("message_datestamp", DB_MESSAGES, "message_from='".$userdata['user_id']."'")) {
			$result = dbquery(
				"SELECT u.user_id, u.user_name, u.name, u.surname, u.user_email, mo.pm_email_notify, COUNT(message_id) as message_count FROM ".DB_USERS." u
				LEFT JOIN ".DB_MESSAGES_OPTIONS." mo USING(user_id)
...
if (!isset($_GET['msg_send']) && !isset($_GET['msg_read']) && $_GET['folder'] != "options") {
	...
	echo "<td width='1%' class='tbl' style='white-space:nowrap;font-weight:".($_GET['folder']=="inbox"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=inbox'>".$locale['402']." [".$bdata['inbox_total']."/".$msg_settings['pm_inbox']."]</a></td>\n";
	echo "<td width='1%' class='tbl' style='white-space:nowrap;font-weight:".($_GET['folder']=="outbox"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=outbox'>".$locale['403']." [".$bdata['outbox_total']."/".$msg_settings['pm_sentbox']."]</a></td>\n";
	echo "<td width='1%' class='tbl' style='white-space:nowrap;font-weight:".($_GET['folder']=="archive"?"bold":"normal")."'><a href='".FUSION_SELF."?folder=archive'>".$locale['404']." [".$bdata['archive_total']."/".$msg_settings['pm_savebox']."]</a></td>\n";
	...
	$result = dbquery(
		"SELECT m.*, u.user_id, u.user_name, u.name, u.surname FROM ".DB_MESSAGES." m
		LEFT JOIN ".DB_USERS." u ON m.message_from=u.user_id
	...
	if (dbrows($result)) {
		...
		echo "<td class='tbl1'><a href='".BASEDIR."profile.php?lookup=".$data['user_id']."'>".$data['surname']." ".$data['name']." (".$data['user_name'].")</a></td>\n</tr>\n";
	...
	if (!isset($_POST['chk_sendtoall']) || $_GET['msg_send'] != "0") {
		...
		$result = dbquery("SELECT u.user_id, u.user_name, u.surname, u.name FROM ".DB_USERS." u ORDER BY user_level DESC, user_name ASC");
		while ($data = dbarray($result)) {
			if ($data['user_id'] != $userdata['user_id']) {
				$sel = ($_GET['msg_send'] == $data['user_id'] ? " selected='selected'" : "");
				$user_list .= "<option value='".$data['user_id']."'$sel>".$data['surname']." ".$data['name']." (".$data['user_name'].")</option>\n";
			}
		}
	}
	...
	if ($_GET['msg_send'] == "0") {
		...
		$udata = dbarray(dbquery("SELECT u.user_id, u.user_name, u.name, u.surname FROM ".DB_USERS." u WHERE user_id='".$_GET['msg_send']."'"));
		echo "<input type='hidden' name='msg_send' value='".$udata['user_id']."' />\n";
		echo "<a href='profile.php?lookup=".$udata['user_id']."'>".$udata['surname']." ".$udata['name']." (".$udata['user_name'].")</a>\n";
	}
