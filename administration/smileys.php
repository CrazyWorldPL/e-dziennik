...
if ((isset($_GET['action']) && $_GET['action'] == "edit") && (isset($_GET['smiley_id'])&& isnum($_GET['smiley_id']))) {
	$result = dbquery("SELECT * FROM ".DB_SMILEYS." WHERE smiley_id='".$_GET['smiley_id']."'");
	if (dbrows($result)) {
		$data = dbarray($result);
		$smiley_code = $data['smiley_code'];
		$smiley_image = $data['smiley_image'];
		$smiley_text = $data['smiley_text'];
		$form_action = FUSION_SELF.$aidlink."&amp;smiley_id=".$data['smiley_id'];
...
$result = dbquery("SELECT * FROM ".DB_SMILEYS." ORDER BY smiley_text");
if (dbrows($result)) {
	$i = 0;
