...
if ($_GET['step'] == "add") {
	if (isset($_POST['add_user'])) {
		...
		if ($error == "") {
			$result = dbquery("INSERT INTO ".DB_USERS." (user_name, name, surname, user_password, user_email, user_hide_email, user_level, user_joined) VALUES ('$username', '".$_POST['name']."', '".$_POST['surname']."', '".md5(md5($_POST['password1']))."', '".$_POST['email']."', '".intval($_POST['hide_email'])."', '104', '".time()."')");
			opentable($locale['480']);
			echo "<div style='text-align:center'><br />\n".$locale['481']."<br /><br />\n";
			echo "<a href='teachers.php".$aidlink."'>".$locale['432']."</a><br /><br />\n";
		...
		echo "</tr>\n<tr>\n";
		echo "<td class='tbl'>".$locale['u014']."</td>\n";
		echo "<td class='tbl'><input type='text' name='name' maxlength='100' class='textbox' style='width:200px;' /></td>\n";
		echo "</tr>\n<tr>\n";
		echo "<td class='tbl'>".$locale['u015']."</td>\n";
		echo "<td class='tbl'><input type='text' name='surname' maxlength='100' class='textbox' style='width:200px;' /></td>\n";
		echo "</tr>\n<tr>\n";
...
elseif ($_GET['step'] == "edit" && isnum($_GET['user_id'])) {
	...
	if (!$user_data || $user_data['user_level'] >= 105) { redirect(FUSION_SELF.$aidlink); }
	if (isset($_POST['savechanges'])) {
		...
		echo "</tr>\n<tr>\n";
		echo "<td class='tbl'>".$locale['u014']."</td>\n";
		echo "<td class='tbl'><input type='text' name='name' value='".$user_data['name']."' maxlength='100' class='textbox' style='width:200px;' /></td>\n";
		echo "</tr>\n<tr>\n";
		echo "<td class='tbl'>".$locale['u015']."</td>\n";
		echo "<td class='tbl'><input type='text' name='surname' value='".$user_data['surname']."' maxlength='100' class='textbox' style='width:200px;' /></td>\n";
		echo "</tr>\n<tr>\n";
...
// Lista
	...
	$result = dbquery("SELECT * FROM ".DB_USERS." WHERE".$user_name." user_status='".$_GET['status']."'AND user_level='104' ORDER BY user_status DESC, user_level DESC, user_name LIMIT ".$_GET['rowstart'].",20");
	echo "<div style='text-align:center;margin-bottom:10px;'>\n";
	echo "<a href='".FUSION_SELF.$aidlink."&amp;".$list_link."&amp;status=0'>".$locale['u060']."</a> ::\n";
	...
	echo "<a href='".FUSION_SELF.$aidlink."&amp;step=add'>".$locale['u061']."</a>\n";
	echo "</div>\n";
	if ($rows) {
		$i = 0;
		...
		while ($data = dbarray($result)) {
			...
			if ($data['user_level'] < 105) echo "<a href='".FUSION_SELF.$aidlink."&amp;step=edit&amp;user_id=".$data['user_id']."'>".$locale['406']."</a>\n";
			if ($data['user_status'] == "2") {
				...
				if ($data['user_level'] < 105) echo "- <a href='".FUSION_SELF.$aidlink."&amp;step=ban&amp;act=on&amp;".$list_link."&amp;status=".$_GET['status']."&amp;rowstart=".$_GET['rowstart']."&amp;user_id=".$data['user_id']."'>".$locale['409']."</a>\n";
			}
			if ($data['user_level'] < 105) echo "- <a href='".FUSION_SELF.$aidlink."&amp;step=delete&amp;".$list_link."&amp;status=".$_GET['status']."&amp;rowstart=".$_GET['rowstart']."&amp;user_id=".$data['user_id']."' onclick='return DeleteMember();'>".$locale['410']."</a>";
			echo "</td>\n</tr>\n"; $i++;
		}
		echo "</table>\n";
	} else {
