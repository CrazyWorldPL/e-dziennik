...
if ($userdata['teacher_subjects'] != "") {
	opentable($locale['r10']);
	
	$classs = explode(".", $userdata['teacher_subjects']);
	...
	while (isset($classs[$i])) {
		$a = explode(":", $classs[$i]);
		$editlist .= "<option value='".$classs[$i]."'>".$a[0]." - ".$sn[$a[1]]."</option>\n";
		$i++;
	}		
	...
	closetable();
  	...
  	opentable($locale['r10']);
  	echo "<div align='center'>".$locale['r20']."</div>";
  	closetable();
}
